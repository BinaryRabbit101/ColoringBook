...
## knob is drawn at the diameter it selects, so the control is its own legend the
## way the dots were.
##
...
## Clearance at each end so the biggest knob cannot draw outside the control.
...
func _draw() -> void:
	...
	draw_arc(Vector2(knob_x, middle), radius + 5.0, 0.0, TAU, 48, KNOB_RING_COLOR, 3.0, true)
