...
- Page complete → the page shows a **persistent** complete state and stays put. The page-flip transition happens when the **player** asks for the next page (BL-4). The last page still reports `book_completed` immediately.
...
- `PageDef`/`BookDef` resources (`scripts/resources/`). A page is `display_image_path` (required — the art the player sees) + optional `mask_image_path` (BL-9: the line art the mapping came from, never loaded or rendered) + `id_map_path` + `regions_json_path`; `get_mapping_source_path()` returns the mask when there is one, the display image otherwise. Books are **discovered**, never preloaded: `BookDef.discover()` scans `res://resources/books/*/book.tres` — adding a book = adding a folder.
...
- **First real art book**: `resources/books/coyote/` + `assets/books/coyote/`. **ONE page** (corrected by BL-9 — it was briefly imported as two): the visible art is the detail drawing, its 2 regions (coyote + paper) come from the outline mask. The artist's full-resolution originals — including that mask — live in `assets/books/coyote/source/` behind a `.gdignore`, and the mask ships with nothing.
...
### Implemented (backlog BL-1..7) — see [docs/BACKLOG.md](../../../docs/BACKLOG.md) for the full notes
...
- **BL-4 no auto-flip**: completion saves, raises the persistent complete state and unlocks the next-page arrow; `ColoringPage.go_to_page()` plays the flip for the forward step off a finished page ONLY, and swaps instantly for every other jump.
...
- **BL-9 display/mask pages**: `PageDef` gained `display_image_path` (renamed from `base_image_path`) + optional `mask_image_path`; `tools/generate_region_map.gd` gained `--display` (map from the mask, write the artifacts next to the page art, resampling the mask to its size). The runtime is unchanged — `PageView` is still handed one visible texture and one ID map. `GameState._entry_for()` now trims a save entry when a book gets SHORTER, so the old two-page coyote save cannot leave statuses or paint PNGs for a page that no longer exists.

**Five smokes must stay green: paint 25, palette 59, flow 104, shell 111, mobile 113.** BL-9 moved two of them: mobile 107 → 113 (the removed coyote page took its 10 per-page assertions with it; check c2 added 16 for the display/mask model, including a stale-import guard on the page texture) and shell 105 → 111 (check h gained the shrinking-book save case).
